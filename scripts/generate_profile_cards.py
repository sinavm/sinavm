...
<rect x="0.5" y="0.5" width="{width-1}" height="{height-1}" rx="12" fill="#0D0B1A" stroke="#2A2750"/>
{body}
</svg>
'''
...
stats = card(360, 200, f'''
<text x="28" y="32" fill="#8B8CF8" font-family="Verdana, sans-serif" font-size="16" font-weight="700">GitHub stats</text>
<text x="28" y="48" fill="#6B6888" font-family="Verdana, sans-serif" font-size="11">@{USER}</text>
{''.join(lines)}
''')
open(f"{OUT}/stats.svg", "w").write(stats)

bar_y = 58
lang_body = [f'<text x="28" y="32" fill="#8B8CF8" font-family="Verdana, sans-serif" font-size="16" font-weight="700">Top languages</text>']
x = 28
...
for name, size in top_langs:
    ...
    ly += 20
langs = card(360, 88 + 20 * len(top_langs) + 8, "\n".join(lang_body))
...
for name, fname in pins:
    ...
    body = f'''
<text x="22" y="32" fill="#8B8CF8" font-family="Verdana, sans-serif" font-size="16" font-weight="700">{esc(name)}</text>
<text x="22" y="58" fill="#C9C6E0" font-family="Verdana, sans-serif" font-size="12">{esc(desc)}</text>
<circle cx="28" cy="88" r="4" fill="{lcolor}"/>
<text x="38" y="92" fill="#A3A0C2" font-family="Verdana, sans-serif" font-size="12">{esc(lang)}</text>
<text x="140" y="92" fill="#A3A0C2" font-family="Verdana, sans-serif" font-size="12">★ {r["stargazerCount"]}</text>
<text x="220" y="92" fill="#A3A0C2" font-family="Verdana, sans-serif" font-size="12">fork {r["forkCount"]}</text>
'''
    open(f"{OUT}/{fname}", "w").write(card(400, 120, body))

streak = card(360, 120, f'''
...
<text x="28" y="96" fill="#EDE9FE" font-family="Verdana, sans-serif" font-size="28" font-weight="700">{contrib:,}</text>
''')
